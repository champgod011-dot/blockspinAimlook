    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character then
            ...
            if root and hum and hum.Health > 0 then
                ...
                -- กัน Anti-Aim teleport spike: ถ้า position กระโดดไกลเกินใน 1 frame ไม่ record
                if #hist > 0 then
                    local lastPos = hist[#hist].pos
                    local dt = os.clock() - hist[#hist].time
                    if dt > 0 and (newPos - lastPos).Magnitude / dt < 500 then
                        table.insert(hist, { time = os.clock(), pos = newPos })
                    end
                else
                    table.insert(hist, { time = os.clock(), pos = newPos })
                end
...
local function predictPosition(part, root)
    ...
    -- Anti-Aim ทำให้ head หมุน ใช้ HRP เป็น base แทน
    local basePos = (root and root.Position) or part.Position
    local parentModel = part.Parent
    local player = parentModel and Players:GetPlayerFromCharacter(parentModel)
    if not player then return basePos end
    local velocity = calculateVelocity(player) or Vector3.zero
    -- clamp velocity ป้องกัน Anti-Aim teleport spike
    local velMag = velocity.Magnitude
    if velMag > 200 then
        velocity = velocity.Unit * 200
    end
    local ping = math.clamp((getPing and getPing()) or 0.1, 0.05, 0.4)
    return basePos + (velocity * ping * VELOCITY_SCALE)
end
        ...
        originalFireServer = hookfunction(SendRemote.FireServer, function(self, ...)
            ...
            if silentAimEnabled and args[2] == "shoot_gun" and aimTarget then
                ...
                if head and root and hum then
                    ...
                    if not originPos then return originalFireServer(self, unpack(args)) end

                    -- คำนวณ predict จาก HRP (ไม่โดน anti-aim spin)
                    local predPos = predictPosition(root, root)

                    -- สร้าง hitbox หลายจุดรอบลำตัว ให้โดนแม้ anti-aim หมุน
                    local offsets = {
                        Vector3.new(0,  0.8, 0),   -- อก
                        Vector3.new(0,  0,   0),   -- กลางตัว
                        Vector3.new(0, -0.5, 0),   -- เอว
                        Vector3.new(0,  1.5, 0),   -- คอ
                        Vector3.new(0.4, 0.5, 0),  -- ข้างซ้าย
                        Vector3.new(-0.4,0.5, 0),  -- ข้างขวา
                    }
                    ...
                    if isShotgun() then
                        args[4] = CFrame.new(originPos, predPos + Vector3.new(0, 0.8, 0))
                        local pellets = {}
                        for i, off in ipairs(offsets) do
                            table.insert(pellets, { [1] = {
                                Instance = head,
                                Normal   = Vector3.new(0, 1, 0),
                                Position = predPos + off,
                            }})
                        end
                        args[5] = pellets
                    else
                        -- เลือก hitpoint ที่ใกล้กับ crosshair มากที่สุด
                        local bestPos = predPos + Vector3.new(0, 0.8, 0)
                        local center  = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
                        local bestDist2D = math.huge
                        for _, off in ipairs(offsets) do
                            local candidate = predPos + off
                            local sp, vis = Camera:WorldToViewportPoint(candidate)
                            if vis then
                        ...
                        local wallBlocked = isBehindWall(originPos, bestPos)
                        args[4] = wallBlocked
                            and CFrame.new(math.huge, math.huge, math.huge)
                            or  CFrame.new(originPos, bestPos)
                        args[5] = { [1] = { [1] = {
                            Instance = head,
                            Normal   = Vector3.new(0, 1, 0),
                            Position = bestPos,
                    ...
                    -- Hit indicator beam
                    pcall(function()
                        local aimPos = args[4] ~= CFrame.new(math.huge, math.huge, math.huge)
                            and (args[5] and args[5][1] and args[5][1][1] and args[5][1][1].Position)
                            or nil
                        if not aimPos then return end
                        local beam = Instance.new("Part")
                        beam.Anchored    = true
                        beam.CanCollide  = false
                        beam.Size        = Vector3.new(0.08, 0.08, (aimPos - originPos).Magnitude)
...
-- Anti-Lock (sky velocity — server sees, client stays still)
local antiLockConn
local antiLockLockedCFrame = nil

local function startAntiLock()
    if antiLockConn then pcall(function() antiLockConn:Disconnect() end) end
    antiLockConn = RunService.Heartbeat:Connect(function()
        if not antiLockEnabled then
            antiLockLockedCFrame = nil
            return
        end
        ...
        -- บันทึก CFrame ตำแหน่งปัจจุบัน (ก่อนดีด)
        if not antiLockLockedCFrame then
            antiLockLockedCFrame = root.CFrame
        end

        -- ยิง velocity สูงให้เซิร์ฟเวอร์เห็น
        root.Velocity = Vector3.new(0, 1e4, 0)

        -- รอ 1 frame แล้วดึงตัวกลับมาที่เดิม (client ไม่เห็นดีด)
        RunService.RenderStepped:Wait()
        if antiLockEnabled and root and root.Parent then
            root.CFrame = antiLockLockedCFrame
...
local AntiLockToggle = CharTab:Toggle({
    ...
    Callback = function(v)
        antiLockEnabled = v
        if not v then antiLockLockedCFrame = nil end
    end,
})
